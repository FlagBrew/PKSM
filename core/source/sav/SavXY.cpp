...
void SavXY::mysteryGift(WCX& wc, int& pos)
{
    ...
    *(u8*)(data + 0x1BC00 + wc6->ID()/8) |= 0x1 << (wc6->ID() % 8);
    std::copy(wc6->rawData(), wc6->rawData() + 264, data + 0x1BD00 + 264*pos);
    pos = (pos + 1) % 24;
}
...
void SavXY::item(Item& item, Pouch pouch, u16 slot)
{
    ...
    switch (pouch)
    {
        ...
        case Medicine:
            ...
            break;
        case Berry:
            std::copy(write.first, write.first + write.second, data + 0xE68 + slot * 4);
            break;
        default:
            return;
    }
...
std::unique_ptr<Item> SavXY::item(Pouch pouch, u16 slot) const
{
    switch (pouch)
    {
        case NormalItem:
        ...
        case Medicine:
            return std::make_unique<Item6>(data + 0xD68 + slot * 4);
        case Berry:
            return std::make_unique<Item6>(data + 0xE68 + slot * 4);
        default:
            return nullptr;
    }
}

std::vector<std::pair<Pouch, int>> SavXY::pouches(void) const
{
    return {
        { NormalItem, 286 },
        { KeyItem, 31 },
...
std::map<Pouch, std::vector<int>> SavXY::validItems() const
{
    return {
        { NormalItem, {
            ...
            498, 499, 500, 537, 538, 539, 540, 541, 542, 543,
            544, 545, 546, 547, 548, 549, 550, 551, 552, 553,
            554, 555, 556, 557, 558, 559, 560, 561, 562, 563,
            564, 571, 572, 573, 576, 577, 580, 581, 582, 583,
            584, 585, 586, 587, 588, 589, 590, 639, 640, 644,
            646, 647, 648, 649, 650, 652, 653, 654, 655, 656,
            657, 658, 659, 660, 661, 662, 663, 664, 665, 666,
            667, 668, 669, 670, 671, 672, 673, 674, 675, 676,
            677, 678, 679, 680, 681, 682, 683, 684, 685, 699,
            704, 710, 711, 715
        }},
        { KeyItem, {
            216, 431, 442, 445, 446, 447, 450, 465, 466, 471,
            628, 629, 631, 632, 638, 641, 642, 643, 689, 695,
            696, 697, 698, 700, 701, 702, 703, 705, 712, 713,
            714
        }},
        { TM, {
        ...
        { Medicine, {
            ...
            53, 54, 134, 504, 565, 566, 567, 568, 569, 570,
            571, 591, 645, 708, 709
        }},
        { Berry, {
...
std::string SavXY::pouchName(Pouch pouch) const
{
    switch (pouch)
    {
        case NormalItem:
