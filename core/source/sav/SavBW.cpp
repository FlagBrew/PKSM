...
void SavBW::mysteryGift(WCX& wc, int& pos)
{
    ...
    *(data + 0x1C900 - 0x100 + pgf->ID()) |= 0x1 << (pgf->ID() & 7);
    std::copy(pgf->rawData(), pgf->rawData() + PGF::length, data + 0x1C900 + pos * PGF::length);
    pos = (pos + 1) % 12;
}

std::string SavBW::boxName(u8 box) const { return StringUtils::getTrimmedString(data, 0x28 * box + 4, 9, (char*)"\uFFFF"); }

void SavBW::boxName(u8 box, std::string name)
{
...
void SavBW::cryptMysteryGiftData()
{
...
std::unique_ptr<WCX> SavBW::mysteryGift(int pos) const
{
...
std::map<Pouch, std::vector<int>> SavBW::validItems() const
{
    return {
        ...
        { KeyItem, {
            437, 442, 447, 450, 465, 466, 471, 504, 533, 574, 578,
            579, 616, 617, 621, 623, 624, 625, 626
        }},
        { TM, {
...
std::string SavBW::pouchName(Pouch pouch) const
{
    switch (pouch)
    {
        case NormalItem:
