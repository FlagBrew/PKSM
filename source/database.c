		...
		if (hidKeysDown() & KEY_A && spriteArray[page*10+currentEntry] != -1) {
			...
			while (aptMainLoop()) {
				...
				if (hidKeysDown() & KEY_START) {
					...
					if (ret != 0 && ret != 1) {
						infoDisp(L"Error during injection!");
						break;
					}
					else if (ret == 1) {
						infoDisp(L"Injection succeeded!");
						break;
					}
			...
			while (aptMainLoop()) {
				...
				if (hidKeysDown() & KEY_START) {
					...
					if ((game == GAME_X || game == GAME_Y) && i == 2048) {
						infoDisp(L"Item not available in XY!");
						break;
					}
...
void massInjector(u8* mainbuf, int game) {
	...
	while(aptMainLoop()) {
		...
		if (hidKeysDown() & KEY_B) {
			if (game == GAME_SUN || game == GAME_MOON) {
				if (confirmDisp(L"Save PokeDex flags?")) {
					u8 pkmn[PKMNLENGTH];
		...
		if (hidKeysDown() & KEY_START) {
			switch (currentEntry) {
				...
				case 6 : {
					...
					free(livingbuf);
					infoDisp(L"Living dex set successfully!");
					break;
				}
				case 7 : {
					char *path[] = {"romfs:/misc/coll_deoxysoblivia.bin"};
					setBoxBin(mainbuf, game, 1, 30, path);
					infoDisp(L"Deoxys collection set successfully!");
					break;
				}
				case 8 : {
					char *path[] = {"romfs:/misc/coll_ranch.bin"};
					setBoxBin(mainbuf, game, 1, 30, path);
					infoDisp(L"My Pokemon Ranch set successfully!");
					break;
				}
				case 9 : {
					char *path[] = {"romfs:/misc/coll_oskorea.bin"};
					setBoxBin(mainbuf, game, 1, 30, path);
					infoDisp(L"Korean collection set successfully!");
					break;
				}
			}
		}
