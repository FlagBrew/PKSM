...
void GUIElementsSpecify(int game) {
	...
	if (game < 6) {
		...
		saveFileEditorBG = sfil_load_PNG_file("romfs:/res/Save File Editor BG.png", SF2D_PLACE_RAM);
		...
		massInjectorBG = sfil_load_PNG_file("romfs:/res/Mass Injector.png", SF2D_PLACE_RAM);
	} else {
...
void GUIElementsExit() {
	...
	sf2d_free_texture(massInjectorBG);
...
void printDB6(int sprite, int i, bool langVett[], bool adapt, bool overwrite, int langSelected, int nInjected) {
	...
	sf2d_start_frame(GFX_BOTTOM, GFX_LEFT);
		...
		sftd_draw_text(fontBold14, 33, 140, RGBA8(0, 0, 0, 200), 14, "Adapt WC to language:");
...
void printEditor(int currentEntry, int langCont, int badgeCont) {
	...
	sf2d_start_frame(GFX_TOP, GFX_LEFT);
		sf2d_draw_texture(saveFileEditorBG, 0, 0);
		...
		for (int i = 0; i < 5; i++) {
			...
			switch (i) {
				...
				case 2 : {
					sftd_draw_text(fontBold12, 18 + (182 - sftd_get_text_width(fontBold12, 12, "Set Balls to max")) / 2, y + 13, DARKGREY, 12, "Set Balls to max");
					break;
				}
				case 3 : {
...
void printPKBank(u8* bankbuf, u8* mainbuf, u8* pkmnbuf, int game, int currentEntry, int box, bool isBank) {
	...
	sf2d_start_frame(GFX_BOTTOM, GFX_LEFT);
		...
		sf2d_draw_texture(left, 2, 11);
		sf2d_draw_texture(right, LRBar->width+7, 11);
		sf2d_draw_texture(topButton, 214, 33);
		sf2d_draw_texture(bottomButton, 214, 65);
		sftd_draw_text(fontBold12, 214 + (106 - sftd_get_text_width(fontBold12, 12, "A: BUFFER")) / 2, 40, BLACK, 12, "A: BUFFER");
		sftd_draw_text(fontBold12, 214 + (106 - sftd_get_text_width(fontBold12, 12, "X: SWITCH B/S")) / 2, 72, BLACK, 12, "X: SWITCH B/S");

		sf2d_draw_texture(pokemonBufferBox, 238, 128);
		u16 n = getPokedexNumber(pkmnbuf);
		if (n) {
			if (isShiny(pkmnbuf))
				sf2d_draw_texture_part(shinySpritesSmall, 250, 137, 40 * (n % 25) + 4, 30 * (n / 25), 34, 30); 
			else 
				sf2d_draw_texture_part(spritesSmall, 250, 137, 40 * (n % 25) + 4, 30 * (n / 25), 34, 30);
		}
...
void printMassInjector(int currentEntry) {
	...
	sf2d_start_frame(GFX_TOP, GFX_LEFT);
		sf2d_draw_texture(massInjectorBG, 0, 0);
