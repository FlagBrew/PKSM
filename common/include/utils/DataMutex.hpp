...
public:
    ...
    private:
        DataLock(T& t, _LOCK_RECURSIVE_T& m) noexcept : rawData(t), mutex(m)
        {
            __lock_acquire_recursive(mutex);
        }
    ...
    template <typename... Args>
    RecursiveDataMutex(Args&&... args) noexcept
        : rawData(std::forward<decltype(args)>(args)...), mutex()
    {
