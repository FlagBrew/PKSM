...
struct Value *ParseFunctionDefinition(struct ParseState *Parser, struct ValueType *ReturnType, char *Identifier, int IsPtrDecl)
{
    ...
    FuncValue = VariableAllocValueAndData(pc, Parser, sizeof(struct FuncDef) + sizeof(struct ValueType *) * ParamCount + sizeof(const char *) * ParamCount, IsPtrDecl, NULL, TRUE);
...
enum ParseResult ParseStatement(struct ParseState *Parser, int CheckTrailingSemicolon)
{
    ...
    switch (Token)
    {
        ...
        case TokenIf:
            ...
            {
                LexGetToken(Parser, NULL, TRUE);
                if (ParseStatementMaybeRun(Parser, (!Condition && !(PreState.Mode == RunModeGoto && Parser->Mode == RunModeRun)), TRUE) != ParseResultOk)
                    ProgramFail(Parser, "statement expected");
            }
            CheckTrailingSemicolon = FALSE;
            break;
