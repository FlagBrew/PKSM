...
namespace
{
    ...
    // RGB will play by different rules, because of course they will. BDSP will be a breath of fresh air.
...
void ExtraSavesSubScreen::drawTop() const
{
    ...
    else if (group != ExtraSavesSubScreen::Group::Pt && group != ExtraSavesSubScreen::Group::E && group != ExtraSavesSubScreen::Group::C && group != ExtraSavesSubScreen::Group::Y)
    {
    ...
    if (updateConfig)
    {
        ...
        else if (group == ExtraSavesSubScreen::Group::RGB)
        {
            ...
            Configuration::getInstance().extraSaves(
                selectedGame ? ((selectedGame == 1) ? ctrIds[14] : ctrIds[15] ) : ctrIds[13], currentSaves);
        }
        else
        {
            if (!addString.empty())
...
void ExtraSavesSubScreen::drawBottom() const
{
    ...
    if (group == ExtraSavesSubScreen::Group::RGB)
    {
        Gui::text(i18n::game(Configuration::getInstance().language(),
            selectedGame
            ? (selectedGame == 1 ? pksm::GameVersion::GN : pksm::GameVersion::BU)
            : pksm::GameVersion::RD),
