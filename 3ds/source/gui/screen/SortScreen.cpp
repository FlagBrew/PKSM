...
SortScreen::SortScreen(bool storage) : storage(storage)
{
    for (int i = 0; i < 5; i++)
    {
        buttons.push_back(std::make_unique<ClickButton>(106, 15 + i * 35, 108, 28, [this, i](){ pickSort(i); return false; }, ui_sheet_button_editor_idx, "", 0.0f, 0));
    }
    buttons.push_back(std::make_unique<ClickButton>(212, 210, 108, 28, [this](){ justSwitched = true; this->sort(); Gui::screenBack(); return true; }, ui_sheet_button_editor_idx, i18n::localize("SORT"), FONT_SIZE_12, COLOR_BLACK));
    buttons.push_back(std::make_unique<ClickButton>(1, 211, 34, 28, [](){ Gui::screenBack(); return true; }, ui_sheet_button_back_idx, "", 0.0f, 0));
}
...
void SortScreen::draw() const
{
    ...
    for (size_t i = 0; i < 5; i++)
    {
        if (i >= sortTypes.size())
        {
            Gui::dynamicText(i18n::localize(std::string(sortTypeToString(NONE))), 160, 29 + 35 * i, FONT_SIZE_12, FONT_SIZE_12, COLOR_BLACK, TextPosX::CENTER, TextPosY::CENTER);
        }
        else
        {
            Gui::dynamicText(i18n::localize(std::string(sortTypeToString(sortTypes[i]))), 160, 29 + 35 *i, FONT_SIZE_12, FONT_SIZE_12, COLOR_BLACK, TextPosX::CENTER, TextPosY::CENTER);
        }
    }
}
