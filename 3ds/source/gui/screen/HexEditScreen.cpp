...
HexEditScreen::HexEditScreen(std::shared_ptr<pksm::PKX> pkm) : pkm(pkm), hid(240, 16)
{
    ...
    for (u32 i = 0; i < pkm->getLength(); i++)
    {
        ...
        buttons[i].push_back(std::make_unique<HexEditButton>(145, 33, 13, 13,
            [this]() { return editNumber(true, true); }, ui_sheet_button_plus_small_idx, "", false,
            0));
        buttons[i].push_back(std::make_unique<HexEditButton>(161, 33, 13, 13,
            [this]() { return editNumber(false, true); }, ui_sheet_button_plus_small_idx, "", false,
            0));
        buttons[i].push_back(std::make_unique<HexEditButton>(145, 75, 13, 13,
            [this]() { return editNumber(true, false); }, ui_sheet_button_minus_small_idx, "",
            false, 0));
                ...
                case 0x16 ... 0x17:
                    if (pkm->generation() != pksm::Generation::SIX)
                    {
                        ...
                        for (int j = 0; j < (i == 0x16 ? 4 : 2); j++)
                        {
                            ...
                            buttons[i].push_back(
                                std::make_unique<HexEditButton>(30, 90 + j * 16, 13, 13,
        ...
        else if (pkm->generation() == pksm::Generation::THREE)
        {
            switch (i)
            {
                ...
                case 0x4D:
                    for (int j = 7; j < 8; j++)
                    {
                        buttons[i].push_back(std::make_unique<HexEditButton>(30, 90 + (j - 7) * 16,
                            13, 13, [this, i, j]() { return this->toggleBit(i, j); },
                            ui_sheet_emulated_toggle_green_idx,
                            i18n::localize(std::string(gen3ToggleTexts[currRibbon])), true, j));
                        buttons[i].back()->setToggled((pkm->rawData()[i] >> j) & 0x1);
            ...
            switch (i)
            {
                ...
                case 0x18 ... 0x19:
                    ...
                    for (int j = 0; j < (i == 0x18 ? 4 : 2); j++)
                    {
                        ...
                        buttons[i].push_back(std::make_unique<HexEditButton>(30, 90 + j * 16, 13,
                            13, [this, currentMark]() { return this->rotateMark(currentMark); },
                            ui_sheet_emulated_toggle_gray_idx,
