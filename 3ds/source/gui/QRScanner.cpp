...
void QRData<Size>::drawThread()
{
    while (aptMainLoop() && !done())
    {
        ...
        // The GUI assigns a depth to each draw. Clear the previous frame's depth too,
        // otherwise editor pixels can occlude the camera preview and scanner controls.
...
void QRData<Size>::handler(std::vector<u8>& out)
{
    ...
    {
        // create cam thread
        if (Threads::atPriority(&captureHelp<Size>, this, 0x10000, 0x1A))
        {
            capturing = true;
        }
        else
...
std::vector<u8> QR_Internal::scan()
{
    ...
    if (Threads::foreground<&QRData<CAMERA_SIZE>::drawThread>(0x10000, data.get()))
    {
        while (!data->done())
        {
            data->handler(out);
