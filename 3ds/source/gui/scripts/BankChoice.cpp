...
BankChoice::BankChoice() : RunnableScreen(nullptr), hid(20, 2), strings(Banks::bankNames())
{
    int newBankNum = 0;
...
void BankChoice::update(touchPosition* touch)
{
    hid.update(strings.size());
    ...
    if (downKeys & KEY_A)
    {
        ...
        if (res.first != Banks::bank->name())
        {
            ...
            Banks::loadBank(res.first, res.second);
        }
        done = true;
        return;
    }
