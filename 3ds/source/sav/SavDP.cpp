...
void SavDP::mysteryGift(WCX& wc, int& pos)
{
    ...
    *(data + 0xA6D0 + gbo + (2047 >> 3)) = 0x80;
    ...
    std::copy(pgt->rawData(), pgt->rawData() + PGT::length, data + 0xA7FC + gbo + pos * PGT::length);
    pos++;
}

std::string SavDP::boxName(u8 box) const { return StringUtils::getString4(data, boxOffset(18, 0) + box*0x28, 9); }

void SavDP::boxName(u8 box, std::string name)
{
...
int SavDP::emptyGiftLocation(void) const
{
    u8 t;
    bool empty;
    u8* wondercards = data + 0xA7FC + gbo;
...
std::map<Pouch, std::vector<int>> SavDP::validItems() const
{
    return {
        { NormalItem, {
...
std::string SavDP::pouchName(Pouch pouch) const
{
    switch (pouch)
    {
        case NormalItem:
